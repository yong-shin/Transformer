...
class Attention(nn.Module):
    ...
    def forward(self, h_src, h_t_tgt, mask=None):
        # |h_src| = (batch_size, length, hidden_size) # 모든 타임스탭의 인코더의 히든스테이트
        # |h_t_tgt| = (batch_size, 1, hidden_size) # 한 타임스탭의 디코더의 히든스테이트
        ...
        # 미니배치별 각 샘플별 디코더의 현재 타임스탭에 대해 인코더의 전체 타임스탭에 대한 웨이트 값이 들어있는 텐서
        
        context_vector = torch.bmm(weight, h_src)
        # |context_vector| = (batch_size, 1, hidden_size)
        # 미니배치별 각 샘플별 디코더의 현재 타임스탭에 대해 어텐션 결과를 얻어 온 컨텍스트 벡터
...
class Encoder(nn.Module):
    ...
    def forward(self, emb):
        ...
        y, h = self.rnn(x)
        # |y| = (batch_size, length, hidden_size) # 전체 타임스탭의 마지막 레이어의 hidden state / 나누기 2, 곱하기 2가 된 것
        # |h[0]| = (num_layers * 2, batch_size, hidden_size / 2) # hidden state를 의미함, cell state는 h[1]
...
class Generator(nn.Module):
    ...
    def forward(self, x):
        ...
        # 미니배치 내 각 샘플 별, 각 타임스텝에 대한 각 단어별 로그 확률값. 
