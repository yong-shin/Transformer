...
class MaximumLikelihoodEstimationEngine(Engine):
    ...
    def train(engine, mini_batch):
        ...
        device = next(engine.model.parameters()).device # 현재 모델의 첫번째 웨이트 파라미터의 device = 현재 모델의 device
        mini_batch.src = (mini_batch.src[0].to(device), mini_batch.src[1]) # 텐서, length(튜플)
        mini_batch.tgt = (mini_batch.tgt[0].to(device), mini_batch.tgt[1])

        # Raw target variable has both BOS and EOS token. 
        # The output of sequence-to-sequence does not have BOS token. 
        # Thus, remove BOS token for reference.
        x, y = mini_batch.src, mini_batch.tgt[0][:, 1:] # 미니배치 내에서 첫번째 BOS는 제외하고 가져와야 함
        # |x| = (batch_size, length)
        # |y| = (batch_size, length)
        ...
        with autocast(not engine.config.off_autocast):
            ...
            loss = engine.crit(
                y_hat.contiguous().view(-1, y_hat.size(-1)), # 배치사이즈*length로 피는 것.
                y.contiguous().view(-1) # 배치사이즈*length로 피는 것.
            )
        ...
        g_norm = float(get_grad_norm(engine.model.parameters())) # gradient norm, 안정적인 학습일수록 점점 작아짐
